...
epochs = args.epochs
this_file = __file__.split('/')[-1].split('.')[0]
saverpath = './model_save/' + this_file + '/'
...
def main():
    ...
    #    layer 1
    W_conv1 = weight_variable(conv_filters[0], name='Weight_conv1', weight_decay=weight_decay)
    b_conv1 = bias_variable([conv_filters[0][3]], name='bias_conv1', weight_decay=weight_decay)
    o_conv1 = tf.nn.relu(
        tf.nn.conv2d(train_batch_x_tensor, filter=W_conv1, strides=[1, 1, 1, 1], padding='VALID') + b_conv1)
    ...
    #     layer2
    W_conv2 = weight_variable(conv_filters[1], name='Weight_conv2', weight_decay=weight_decay)
    b_conv2 = bias_variable([conv_filters[1][3]], name='bias_conv2', weight_decay=weight_decay)
    ...
    W_fc3 = weight_variable([conv_layers_out_flatten, nets_cf[0]], name='Weight_fc3', weight_decay=weight_decay)
    b_fc3 = bias_variable([nets_cf[0]], name='bias_fc3', weight_decay=weight_decay)
    ...
    W_fc4 = weight_variable([nets_cf[0], nets_cf[1]], name='Weight_fc4', weight_decay=weight_decay)
    b_fc4 = bias_variable([nets_cf[1]], name='bias_fc4', weight_decay=weight_decay)
    ...
    W_fc5 = weight_variable([nets_cf[1], nets_cf[2]], name='Weight_fc5', weight_decay=weight_decay)
    b_fc5 = bias_variable([nets_cf[2]], name='bias_fc5', weight_decay=weight_decay)
    tf.summary.histogram('W_fc5', W_fc5)
    tf.summary.histogram('b_fc5', b_fc5)
    o_out5 = tf.nn.softmax(tf.matmul(o_out4, W_fc5) + b_fc5)
    #     loss function
    cross_entropy = -tf.reduce_mean(train_batch_y * tf.log(o_out5 + 1e-8))
    tf.add_to_collection("wd_loses", cross_entropy)
    lose = tf.add_n(tf.get_collection("wd_loses"))
    tf.summary.scalar(name="loss", tensor=lose)
    ...
    if not os.path.exists('./tensorboards/' + this_file + '/'):
        os.mkdir('./tensorboards/' + this_file + '/')
    writer = tf.summary.FileWriter(logdir='./tensorboards/' + this_file + '/', graph=sess.graph)
